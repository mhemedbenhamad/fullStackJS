...
const selectEtudiants = async (req, res) => {
  try {
    ...
    res.header(
      ...
      "Content-Type, Access-Control-Allow-Headers, Access-Control-Allow-Methods, Authorization, X-Requested-With"
    );
    res.json({
      status: 201,
...
export const SignInStudent = async (req, res) => {
  ...
  if (email && password) {
    ...
    if (!student) {
      res
        .status(401)
        .json({ message: "No such student found ", student: email });
    }
